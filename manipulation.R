...
fac11<-read.csv("Faculty Survey 4 Wave Data 1.0.csv",stringsAsFactors=TRUE)
...
#roups<-names(table(fac17$contact_group))
...
qid_overtime<-read.csv("FacultySurvey_QIDovertime.csv",stringsAsFactors=TRUE)
names(qid_overtime)
...
#lapply(1:6, function(i) write.csv(new_surveys[[i]], paste0(names(new_surveys[i]), ".csv"), row.names=FALSE))

## can't standardize qid, have to reference qid_overtime sheet when making graphs ##
